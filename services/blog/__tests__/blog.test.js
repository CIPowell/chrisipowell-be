...
describe("Listing Blog articles", () => {
  ...
  beforeEach(() => {
    ...
    contentful.getEntries.mockReturnValue({
      items: [testEntry, testEntry, testEntry],
    });
  });
  ...
  it("Should request a list of blog articles", async () => {
    ...
    let [{ skip, limit, order, content_type }] =
      contentful.getEntries.mock.calls[0];
  ...
  `(
    "Should request $expLimit entries, skipping $expSkip when asking for page $page with a pagesize of $pageSize",
    async ({ page, pageSize, expLimit, expSkip }) => {
  ...
  it("should return a url for the videof there is one", async () => {
    ...
    items.forEach(({ video }) => {
      expect(video).toBe(
        `https://stream.mux.com/vdija4nm02TW5kleRaMld8XavCjaMDCSKGZfMjyKNfkA.m3u8`
      );
    });
  });
});
